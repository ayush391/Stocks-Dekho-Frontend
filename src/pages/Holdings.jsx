    ...
    return (
        <Card sx={{borderRadius:10 , marginBottom:'1rem' , padding:2}}>
            ...
            <Stack direction={"row"} justifyContent={'space-evenly'} >
                ...
                <Typography sx={{verticalAlign:'center' , textAlign:'center'}}>{holdings.meta?holdings.meta.companyName:holdings.stockSymbol}</Typography>
                <Stack direction={"column"}>
                <Typography fontWeight={'bold'} color={priceTick.pChange.substring(0,1) == '-'?'red':'lightgreen'}>{priceTick.lastPrice} </Typography>
                <Typography fontSize={'0.8rem'} textAlign='center' color={priceTick.pChange.substring(0,1) == '-'?'red':'lightgreen'}>{priceTick.pChange}</Typography>
                </Stack>
                <Stack direction={"column"}>
...
export const Holdings=()=>{
    ...
    return(
        <div >
            {
                HoldingsList.map((holdings)=>(
                    <StockCard holdings={holdings}/>
                ))
            }
        </div>
    )
